...
public class zTurretLimelightCommand extends CommandBase {
     ...
     */
    public zTurretLimelightCommand(TurretSubsystem turret, Limelight limelight, double speed, double minSpeed,
            double range, double tolerance) {
        ...
        switch (m_state) {
            case ONTARGET:
                if (onTarget() && m_limelight.isTargetAvailible()) { // make sure we are stil on target
                    ...
                    // if ((m_drive.wheelsHavePower()==false) && (m_TurretSubsystem.isMovingLeft(0)==false) &&
                    // (m_TurretSubsystem.isMovingRight(0)==false)){
                    // m_shooterStart=Timer.getFPGATimestamp();
                    // m_state=CommandState.SHOOTING;
                    // }
                }
                else {
                    System.out.println("Setting State to Searching - OnTarget:" + onTarget() + " - TargetAvailable:"
                            + m_limelight.isTargetAvailible());
