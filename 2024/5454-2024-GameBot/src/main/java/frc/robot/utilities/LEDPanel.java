...
import frc.robot.Constants.LEDPanel.Colors;
...
public class LEDPanel {
    ...
    private static final int [] shapeVCone = {3,4,18,21,29,33,38,41,46,48,49,50,51,52,53,54,55};
        ...
        m_upperLEDLimit=leds-1; //0 based array so offset by 1
        m_led.setData(m_ledBuffer);
        m_led.start();
    }

    public LEDPanel(int port, int leds, int start) {
        // initialize all relevant objects
        m_led = new AddressableLED(port);
        this.start = start;
