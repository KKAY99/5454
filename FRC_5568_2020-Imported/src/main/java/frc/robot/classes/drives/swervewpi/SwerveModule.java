...
public class SwerveModule {
    ...
    private int m_desiredEncoderPositon = 0;
    ...
    public void setDesiredState(SwerveModuleState state) {
        ...
        double driveOutput = m_drivePIDController.calculate(m_driveEncoder.getVelocity(),
                state.speedMetersPerSecond);

        double driveFeedforward = m_driveFeedforward.calculate(state.speedMetersPerSecond);
        System.out.println("Voltage=" + (driveOutput) + " + " + (driveFeedforward) + " for speed of " + state.speedMetersPerSecond);
        m_driveMotor.setVoltage(driveOutput + driveFeedforward);
        // Calculate the turning motor output from the turning PID controller.
        // Angle state.angle.getRadians()

        int ticksPerRotation = 1024; // in encoder counts
        
        int currentPosition =(int)  m_turningMotor.getSelectedSensorPosition(Constants.kSlotIdx);
        m_desiredEncoderPositon = (int) Math.round(state.angle.getDegrees() * ticksPerRotation / 360.0) + m_moduleOffset;

        //int newPosition = (int) MathUtil.minChange(m_desiredEncoderPositon, currentPosition, ticksPerRotation) + currentPosition;
        int newPosition=m_desiredEncoderPositon;
        System.out.println("Turning to " + state.angle.getDegrees() + " from " + currentPosition + " to "  + m_desiredEncoderPositon + " via " + newPosition );
    ...
    public int getSteerEncoderTarget() {
        return (int) m_desiredEncoderPositon;
    }
    public double getSteerAngle(){
		return (m_turningMotor.getSelectedSensorPosition(0) * kWheelCircumference);
	}
    ...
    public void zeroOutTurningEncoder(){
