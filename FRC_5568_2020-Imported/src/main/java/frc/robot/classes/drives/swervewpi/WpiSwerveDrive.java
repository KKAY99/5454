...
public class WpiSwerveDrive {
    ...
    //public static final int backLeftOffset = -628;
    //public static final int backRightOffset =98;
   ...
   private SlewRateLimiter m_rotLimiter = new SlewRateLimiter(3);
    ...
    public void drive(double xSpeed, double ySpeed, double rot, boolean fieldRelative) {
        ...
        System.out.println(swerveModuleStates[0].speedMetersPerSecond + " -fr " + swerveModuleStates[1].speedMetersPerSecond + " -bl " + swerveModuleStates[2].speedMetersPerSecond + " -br " + swerveModuleStates[3].speedMetersPerSecond);
    ...
    public void driveWithJoystick(double RCW, double FWD, double STR, boolean fieldCentric,
        ...
        public void move( double direction, double speed, double distance, boolean stopAtEnd) {
            ...
            xSpeedPercent=Math.cos(direction);
            ySpeedPercent=Math.sin(direction);
            
            startPosition=m_frontRight.getDriveEncoder();
            targetPosition=distance+startPosition;
            while(targetPosition>=m_frontRight.getDriveEncoder()){
                System.out.println("targetdistance -" + targetPosition + " - currentdistance" + m_frontRight.getDriveEncoder());
                System.out.println("xSpeedPer-" + xSpeedPercent + " -ySpeedPer " + ySpeedPercent);
                drive(xSpeedPercent*speed,ySpeedPercent*speed,rotation,false);
            }
            if (stopAtEnd){

                driveKill();
                //because pid won't continue to loop manually stop all wheels
                m_frontRight.spinWheel(0);
                m_frontLeft.spinWheel(0);
                m_frontRight.spinWheel(0);
                m_frontRight.spinWheel(0);
            }
    ...
    public void driveKill() {
       ...
       drive((double) 0, (double) 0, (double) 0, false);
    ...
    public double getDriveEncoder (int swerveModule) {
        double returnValue=0;
        switch (swerveModule){
            
            case SwerveModules.kFrontLeft: {
                returnValue= m_frontLeft.getDriveEncoder();    
                break;
            }
            case SwerveModules.kFrontRight: {
                returnValue= m_frontLeft.getDriveEncoder();  
     ...
     */
     ...
     public void zeroOutSwerve(){
         m_frontRight.zeroOutTurningEncoder();
         m_frontLeft.zeroOutTurningEncoder();
         m_backRight.zeroOutTurningEncoder();
         m_backLeft.zeroOutTurningEncoder();
     }
